...
$qry="SELECT bloodgroup, SUM(bloodqty) AS total_quantity
FROM blood
GROUP BY bloodgroup";
$result= mysqli_query($conn,$qry);
$minQuantity = 500; // Define the minimum quantity
$lowQuantities = array(); // Array to store blood groups with low quantities
...
<head>
    ...
    function drawChart() {
        // Retrieve the query result again
        <?php
        $result = mysqli_query($conn, $qry);
        ?>
        ...
        var data = google.visualization.arrayToDataTable([
            ...
            <?php
            while ($row = mysqli_fetch_array($result)) {
                echo "['" . $row['bloodgroup'] . "', " . $row['total_quantity'] . "],";
            }
            ?>
        ]);
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <!-- /.row -->
            <?php
            if (!empty($lowQuantities)) {
                $message = "Alert !!! The blood volume of the following groups has reached the minimum limit : " . implode(", ", $lowQuantities);
                echo "<div class='alert alert-danger alert-highlight'>$message</div>";
            }
            ?>
            <div class="row">
